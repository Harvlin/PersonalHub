...
function Sidebar({ onSearch }: { onSearch: () => void }) {
  const { projects, signedOut, setSignedOut } = useStore();
        ...
        <div className="relative mt-3">
          {popover ? (
            <div className="hairline absolute bottom-11 left-0 w-full rounded-md bg-paper p-1">
              ...
              >
                {signedOut ? "Sign back in" : "Sign out"}
              </button>
            </div>
          ) : null}
          <button
          ...
          >
            ...
            <span className="text-[12.5px] font-medium">Harvlin</span>
            <span className="mono ml-auto text-[11px] text-ink-muted">@harvlin</span>
          </button>
        </div>
      </div>
    </aside>
